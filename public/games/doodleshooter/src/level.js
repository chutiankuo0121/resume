// 单人街区：合并墨线几何与轴对齐碰撞体，并用碰撞体生成寻路网格。
...
function createBuilder(scene, world) {
  const geos = {}; const L = { rings: [], spawns: [], snipers: [], pickups: [], animated: [], meshes: [], playerStart: new THREE.Vector3(0, 0, 42), bounds: { minX: -55, maxX: 55, minZ: -55, maxZ: 55 }, grappleMovers: [], breakables: [] };
...
function buildDistrict(B) {
  ...
  // ---------------- ground + perimeter ----------------
  // 单人街区保持紧凑边界、中央高塔和原版抓钩路线。
  const P = 55, T = 6, PH = 18, E = P - 3.8, D = P - 3;
  ...
  // ---------------- building A (west): 3 floors, fire escape, ruler bridge to the tower ----------------
  {
    ...
    // 尺子桥从 A 栋屋顶通向中央高塔三层，高度为 12。
    { const bx2 = -7; const len = bx2 + 25.2;
  ...
  planes(3, 30, 30, { rStep: 8, hStep: 6, scale: 1.4 });
  return B.finish();
}
...
export function buildLevel(scene, world) {
  return buildDistrict(createBuilder(scene, world));
}
