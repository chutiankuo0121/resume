...
class Projectiles {
  ...
  fire(pos, dir, speed, dmg, owner, ink = INK.RED, thick = 0.045, blast = 0) {
    ...
    const p = { pos: pos.clone(), prev: pos.clone(), vel: dir.clone().multiplyScalar(speed), dmg, owner, life: 4, deflected: false, ink, thick, origin: pos.clone(), blast };
    this.list.push(p);
    return p;
  }
  ...
  update(dt) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (!p.deflected) {
        // 敌方弹体只检测当前玩家；先判定刀刃招架，再判定身体受伤。
        const P = ctx.player;
        if (P.alive && this._segHitsPlayer(p.prev, p.pos, P, Math.max(p.blast ? 0.9 : 0.5, P.blockRadius))) {
...
export class EnemyManager {
  ...
  _explodeBomber(e, scale = 1) {
    ...
    { const P = this.ctx.player; const d = P.center.distanceTo(c); if (P.alive && d < T.blast * scale) { P.takeDamage(T.dmg * this.mods.damage * Math.sqrt(1 - d / (T.blast * scale)), c); P.knockback(_v.subVectors(P.center, c).normalize(), 7); } }
  ...
  update(dt) {
    ...
    for (const e of this.enemies) {
      ...
      const P = ctx.player; const pp = P.body.pos, pc = P.center;
    ...
    if (e.bossAtk) {
      ...
      if (a.kind === 'stomp') {
        if (a.t > 0.75 && !a.done) {
          ...
          { const t = this.ctx.player; const dd = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z); if (t.alive && dd < 7.5 && t.body.pos.y < b.pos.y + 2.5) { t.takeDamage(T.dmg * this.mods.damage, e.center); t.knockback(_d.subVectors(t.center, e.center).normalize(), 9); } }
    ...
    if (e.bossAtk) {
      ...
      if (a.kind === 'charge') {
        if (a.t < 0.7) { e.yawT = yawTo; b.vel.x = damp(b.vel.x, 0, 8, dt); b.vel.z = damp(b.vel.z, 0, 8, dt); if (a.t > 0.55 && !a.dir) { a.dir = new THREE.Vector3(pp.x - b.pos.x, 0, pp.z - b.pos.z).normalize(); audio.bossRoar(e.center); } }
          ...
          { const t = this.ctx.player; const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z); if (t.alive && d < 2.6 && Math.abs(t.body.pos.y - b.pos.y) < 3 && !a.hit) { a.hit = true; t.takeDamage(T.dmg * this.mods.damage, e.center); t.knockback(_d.subVectors(t.center, e.center).setY(0.2).normalize(), 13); ctx.effects.shakeAmt += 0.5; } }
        ...
        if (a.t > 0.9 && !a.done) {
          ...
          { const t = this.ctx.player; const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z); if (t.alive && d < 6.5 && t.body.pos.y < b.pos.y + 3) { t.takeDamage(T.dmg * 0.8 * this.mods.damage, e.center); t.knockback(_d.subVectors(t.center, e.center).normalize(), 9); } }
        }
  ...
  _thinkInkblot(e, dt, pp, pc, dist, dy, yawTo, P) {
    ...
    if (e.hopping && b.onGround) { e.hopping = false; audio.stomp(e.center); ctx.effects.shakeAmt += 0.4; ctx.effects.bloodPool(b.pos, 3.5, INK.BLACK); { const t = this.ctx.player; const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z); if (t.alive && d < 4.5 && Math.abs(t.body.pos.y - b.pos.y) < 2.5) { t.takeDamage(T.dmg * 0.7 * this.mods.damage, e.center); t.knockback(_d.subVectors(t.center, e.center).normalize(), 8); } } }
