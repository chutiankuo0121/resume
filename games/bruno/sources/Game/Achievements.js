...
export class Achievements
{
    constructor()
    {
        ...
        const areaNames = new Set(locations.map(item => item.respawnName))
        ...
        for(const groupName in localAchievements)
        {
            ...
            if(group)
            {
                // 存档中的区域只能属于当前地图，已删区域不能抵扣新的探索进度。
                const progress = groupName === 'areas'
                    ? (Array.isArray(localAchievements[groupName]) ? localAchievements[groupName] : []).filter(name => areaNames.has(name))
                    : localAchievements[groupName]
                group.setProgress(progress, true)
            }
        }

        this.globalProgress.update()
        // 先恢复有效进度，再判断旧的全成就时间；初始化空分组时不能提前清零。
        if(!this.globalProgress.achieved && this.globalProgress.timeEnd)
        {
            this.globalProgress.timeEnd = 0
            localStorage.removeItem('astra:bruno:achievementsTimeEnd')
        }
        this.rewards.update()
        this.storage.save()
    }
    ...
    setStorage()
    {
        ...
        this.storage.save = () =>
        {
            ...
            localStorage.setItem('astra:bruno:achievements', encodedData)
        }

        this.storage.get = () =>
        {
            const localAchievements = localStorage.getItem('astra:bruno:achievements')
    ...
    setGlobalProgress()
    {
        ...
        let localTimeStart = localStorage.getItem('astra:bruno:achievementsTimeStart')
        if(localTimeStart)
        {
        ...
        let localTimeEnd = localStorage.getItem('astra:bruno:achievementsTimeEnd')
        if(localTimeEnd)
        {
        ...
        this.globalProgress.update = () =>
        {
            ...
            if(this.globalProgress.achievedCount === this.globalProgress.totalCount)
            {
                ...
                {
                    // Not already ended
                    if(!localStorage.getItem('astra:bruno:achievementsTimeEnd'))
                    {
                        this.globalProgress.timeEnd = this.game.player.timePlayed.all
                        localStorage.setItem('astra:bruno:achievementsTimeEnd', this.globalProgress.timeEnd)
                    }
        ...
        this.globalProgress.reset = () =>
        {
            ...
            this.globalProgress.timeEnd = 0
            localStorage.setItem('astra:bruno:achievementsTimeStart', this.globalProgress.timeStart)
            localStorage.removeItem('astra:bruno:achievementsTimeEnd')
    ...
    setRewards()
    {
        ...
        const localRewardName = localStorage.getItem('astra:bruno:achievementsReward')
        if(localRewardName)
        {
        ...
        this.rewards.set = (itemName) =>
        {
            ...
            if(item && !item.locked && item !== this.rewards.current)
            {
                ...
                // Save
                localStorage.setItem('astra:bruno:achievementsReward', item.name)
