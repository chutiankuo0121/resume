...
export class VisualVehicle
{
    constructor(model)
    {
        ...
        // 车辆装配会重新挂载底盘；克隆场景，避免再次换车时缓存模型被拆空。
    ...
    destroy()
    {
        ...
        if(this.blinkers)
        {
            ...
            gsap.killTweensOf(this.blinkers.blink)
        }

        this.game.achievements.events.off('rewardActiveChange', this.paints.rewardCallback)
        this.boostTrails.instance.destroy()
        this.backLights.material.dispose()
        this.antenna?.head.removeFromParent()
    ...
    setPaints()
    {
        ...
        this.paints.choices.orange = this.game.materials.createGradient('orangeGradient', '#ff940d', '#af0071')
        this.paints.choices.white = this.game.materials.createGradient('whiteGradient', '#ffffff', '#b5b5b5')
        this.paints.choices.black = this.game.materials.createGradient('blackGradient', '#626262', '#262526')
        ...
        this.paints.rewardCallback = (reward) =>
        {
            this.paints.changeTo(reward.name)
        }
        this.game.achievements.events.on('rewardActiveChange', this.paints.rewardCallback)
    }
    ...
    setAntenna()
    {
        ...
        this.antenna.head = this.model.getObjectByName('antennaHead')
