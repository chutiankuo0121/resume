...
import { attribute, clamp, Fn, fract, max, mod, step, uniform, vec2, vec3 } from 'three/tsl'
...
export class RainLines
{
    constructor()
    {
        ...
        // 雨量控制密度，积雪降低长度与落速，风力控制倾斜。
        this.updateVisibleRatio = () =>
        {
            this.visibleRatio.value = Math.pow(this.game.weather.rain.value, 2)
        }

        this.updateLengthRatio = () =>
        {
            ...
            this.length.value = lerp(baseLength, snowLength, snowRatio)
        }
        ...
        this.updateSpeedRatio = () =>
        {
            ...
            this.speed = lerp(baseSpeed, snowSpeed, snowRatio)
        }

        this.updateInclineRatio = () =>
        {
            this.incline.value = remapClamp(this.game.weather.wind.value, 0, 1, 0.1, 0.4)
        }
    }
