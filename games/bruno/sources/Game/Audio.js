...
export class Audio
{
    ...
    setAmbiants()
    {
        ...
        // Oven fire (Project Area + Cookie Area)
        {
            ...
            if(this.game.world.areas?.projects)
                positions.push(this.game.world.areas.workshop.references.items.get('oven')[0].position)
        ...
        // Campfire (Lab Area + Bonfire Area)
        {
            ...
            if(this.game.world.areas?.lab)
                positions.push(this.game.world.areas.machinery.references.items.get('fire')[0].position)
    ...
    setMute()
    {
        ...
        this.mute.activate = () =>
        {
            ...
            localStorage.setItem('astra:bruno:soundToggle', '1')
        ...
        this.mute.deactivate = () =>
        {
            ...
            Howler.mute(false)
            this.mute.active = false
            localStorage.setItem('astra:bruno:soundToggle', '0')
            document.documentElement.classList.remove('is-audio-muted')
            this.events.trigger('muteChange', [ false ])
        }

        // From local storage
        const soundToggleLocal = localStorage.getItem('astra:bruno:soundToggle')
        if(soundToggleLocal !== null && soundToggleLocal === '1')
            this.mute.activate()

        // Inputs keyboard
        this.game.inputs.addActions([
            { name: 'mute', categories: [ 'intro', 'modal', 'menu', 'racing', 'wandering' ], keys: [ 'Keyboard.l' ] },
        ])
