...
| `src/lib/loading/drawPrelude.ts` | 复用首页渲染器的星链进度条、C 形轮廓与揭幕合成 |
...
加载条与 C 形揭幕共用拼接线的 `portalLight` 光效、星点片元和 Bloom 参数。放大时只改变轮廓，粒子大小保持屏幕像素尺度；揭幕后释放临时渲染资源。弯折时添加最高 1.6px 的轻微高斯模糊，推进至 90% 时完全清晰；减少动态效果模式使用直接淡入。
