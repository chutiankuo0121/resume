...
export function useWorkDetail() {
  ...
  type Detail = { Component: Modules["Component"]; work: NonNullable<ReturnType<Modules["works"]["get"]>>; origin?: CardOrigin };
  const [detail, setDetail] = useState<Detail | null>(null);
  const [error, setError] = useState("");
  const request = useRef(0);
  useEffect(() => () => { request.current++; }, []);
  const clear = useCallback(() => { request.current++; setDetail(null); setError(""); }, []);
  const open = useCallback(async (id: string, origin?: CardOrigin) => {
    ...
    try {
      ...
      if (request.current === current) setDetail({ Component, work, origin });
    } catch {
